...
export default function SSLToolkitClient() {
  ...
  const handleDownloadAll = async () => {
    ...
    const zip = new JSZip();
    const readme = [`# SSL PACKAGE SUMMARY - ${baseName}`, "", "Components Included:", detected.cert ? `- Certificate` : "", detected.key ? `- Private Key` : "", detected.chain.length > 0 ? `- CA Bundle (${detected.chain.length} certs)` : "", "", "Ready for Nginx, Apache, IIS, and Java environments."].join("\n");
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="mt-8 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 opacity-60">
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest">Toolsy OS Native</p>
          </div>
        </div>
      </div>
